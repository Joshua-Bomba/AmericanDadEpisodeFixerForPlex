...
namespace AmericanDadEpisodeFixerForPlex
{
    ...
    public class EpisodeMetaDataHandler : IAsyncDisposable
    {
        ...
        private Dictionary<string, EpisodeMetaData> _episodes;
        public EpisodeMetaDataHandler() 
        {
            _episodes =new Dictionary<string, EpisodeMetaData>();
        }
        ...
        private async IAsyncEnumerable<EpisodeMetaData> ProcessMatch(int season, string table)
        {
                        ...
                        case XmlNodeType.EndElement:
                            ...
                            if (TABLE_TAGS.Contains(res))
                            {
                                ...
                                if (res == "td")
                                {
                                    if(row == null)
                                    {
                                        row = new EpisodeMetaData { Season = season };
                                    }
        ...
        public async ValueTask<bool> CheckForCachedEpisodes()
        {
            if(File.Exists(CachedEpisodes))
            {
                ...
                _episodes = JsonConvert.DeserializeObject<Dictionary<string, EpisodeMetaData>>(content);
                return true;
            }
            return false;
        }
        ...
        public async ValueTask<bool> ProcessEpisodeData()
        {
            if(!await this.CheckForCachedEpisodes())
            {
                ...
                if (!string.IsNullOrWhiteSpace(content))
                {
                    ...
                    {
                        await foreach (EpisodeMetaData e in ProcessMatch(++season, m.Value))
                        {
        ...
        public Dictionary<int,Dictionary<int,EpisodeMetaData>> GetEpisodesForEachSeason()
        {
